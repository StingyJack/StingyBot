    ...
    private void CopyTo(string[] array, int arrayIndex)
    {
      ...
      else
      {
        ...
        if (array.Length - arrayIndex < 1)
          throw new ArgumentException(string.Format("'{0}' is not long enough to copy all the items in the collection. Check '{1}' and '{2}' length.", new object[3]{ (object) "array", (object) "arrayIndex", (object) "array" }));
